...
Patch_class="/Users/barry/Documents/workspace/jdt_core_patched_from_new/bin"
...
function getName() {
	for file in $(find $1 -name '*.java' -print);do
		...
		echo "------------------------src path-----------------------------" >> "single_log.txt"
		echo "old_src: "$old_src >> "single_log.txt"
		echo "patch_src: "$patch_src >> "single_log.txt" 2>&1
		echo "new_src: "$new_src >> "single_log.txt"

		#eval AST Differ
		echo "--------------------running Diff new-------------------------" >> "single_log.txt"
		eval $Diff_new >> "single_log.txt" 2>&1
		echo "--------------------Diff new done----------------------------" >> "single_log.txt"
		echo "--------------------running Diff patch-----------------------" >> "single_log.txt"
		eval $Diff_patch >> "single_log.txt" 2>&1
		echo "--------------------Diff patch done--------------------------" >> "single_log.txt"
		...
		echo "--------------------make jpf new-----------------------------" >> "single_log.txt"
		#output new jpf
		...
		echo "--------------------jpf new done-----------------------------" >> "single_log.txt"

		#output patch jpf
		echo "--------------------make jpf patch---------------------------" >> "single_log.txt"
		...
		echo "--------------------jpf patch done---------------------------" >> "single_log.txt"
